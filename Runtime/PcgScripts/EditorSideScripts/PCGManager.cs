        ...
        public void LoadMainAlgo()
        {
            ...
            switch (mainAlgorithm)
            {
                case MainAlgo.VORONI:
                    {
                        var comp = this.gameObject.AddComponent<VoronoiMA>();
                        comp.InspectorAwake();
                    }
                    break;
                case MainAlgo.RANDOM_WALK:
                    {
                        var comp = this.gameObject.AddComponent<RandomWalkMA>();
                        comp.InspectorAwake();
                    }
                    break;
                case MainAlgo.ROOM_GEN:
                    {
                        var comp = this.gameObject.AddComponent<RanRoomGenMA>();
                        comp.InspectorAwake();
                    }
                    break;
                case MainAlgo.CELLULAR_AUTOMATA:
                    {
                        var comp = this.gameObject.AddComponent<CellularAutomataMA>();
                        comp.InspectorAwake();
                    }
                    break;
                case MainAlgo.L_SYSTEM:
                    {
                        var comp = this.gameObject.AddComponent<LSystem>();
                        comp.InspectorAwake();
                    }
                    break;
                case MainAlgo.DELUNARY:
                    {
                        var comp = this.gameObject.AddComponent<DelunaryMA>();
                        comp.InspectorAwake();
                    }
                    break;
                case MainAlgo.WFC:
                    {
                        this.gameObject.AddComponent<WFCRuleDecipher>();

                        var comp = this.gameObject.AddComponent<NewWFCAlog>();
                        comp.InspectorAwake();
                    }
                    break;
                case MainAlgo.PERLIN_NOISE:
                    {
                        var comp = this.gameObject.AddComponent<PerlinNoiseMA>();
                        comp.InspectorAwake();
                    }
                    break;
                case MainAlgo.PERLIN_WORM:
                    {
                        var comp = this.gameObject.AddComponent<PerlinWormsMA>();
                        comp.InspectorAwake();
                    }
                    break;
                case MainAlgo.DIAMOND_SQUARE:
                    {
                        var comp = this.gameObject.AddComponent<DiamondSquareMA>();
                        comp.InspectorAwake();
                    }
                    break;
                case MainAlgo.DIFF_LIM_AGGR:
                    {
                        var comp = this.gameObject.AddComponent<DiffLimAggMA>();
                        comp.InspectorAwake();
                    }
                    break;
                case MainAlgo.GENERATE_DUNGEON:
                    {
                        var comp = this.gameObject.AddComponent<LoadMapMA>();
                        comp.InspectorAwake();
                    }
                    break;
                default:
